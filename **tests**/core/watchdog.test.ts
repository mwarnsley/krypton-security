...
let alertErrorHandler: ((error: NodeJS.ErrnoException) => void) | undefined;
let workspaceEventHandler: ((eventType: string, filename: string | null) => void) | undefined;
let workspaceErrorHandler: ((error: Error) => void) | undefined;
const alertWriteMock = vi.fn(() => true);
const alertStreamStub = {
  on: vi.fn((eventName: string, listener: (error: NodeJS.ErrnoException) => void) => {
    if (eventName === 'error') {
...
beforeAll(async () => {
  createWriteStreamSpy = vi.spyOn(fs, 'createWriteStream').mockReturnValue(alertStreamStub);
...
describe('watchdog core engine', () => {
  ...
  it('fails closed when path resolution receives an invalid runtime token', () => {
    expect(watchdog.verifyPathAccess(null as unknown as string)).toBe(false);
  });
  ...
  test.each([0, -1, 1.5, Number.NaN])('rejects invalid process ID %s', (pid) => {
    const processKillSpy = vi.spyOn(process, 'kill').mockReturnValue(true);

    try {
      expect(() => watchdog.quarantineProcess(pid, ILLEGAL_PATH)).toThrow(RangeError);
    } finally {
      processKillSpy.mockRestore();
    }
  });

  test.each([0, -1, 1.5, Number.NaN])('rejects invalid process registration %s', (pid) => {
  ...
  it('propagates process termination failures', () => {
    ...
    try {
      ...
      expect(() => watchdog.quarantineProcess(42_426, ILLEGAL_PATH)).toThrow(
  ...
  describe('workspace watcher', () => {
    it('starts one persistent recursive watcher for the sandbox', () => {
      const watchSpy = vi.spyOn(fs, 'watch').mockReturnValue(workspaceWatcherStub);
    ...
    it('rejects a watcher path outside the sandbox', () => {
      expect(() => watchdog.startWorkspaceWatcher('../other-workspace')).toThrow(RangeError);
    });
