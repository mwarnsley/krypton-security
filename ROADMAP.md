...
**Status:** In progress; source-checkout initial-child supervisor implemented.
Standalone distribution, descendant containment, and host/network adapters remain planned.
...
- [ ] Establish a measured time-to-first-containment target below 30 seconds from
      the start of installation to a successfully protected `krypton run`
      session on a supported clean workstation. This is a target, not a current
      guarantee.

### Wrap & Run CLI Wrapper (`krypton run`)

- [x] Provide one command that starts an explicitly registered initial tool or autonomous
...
- [x] Discover and validate the configured protected workspace, authenticate daemon
      health before spawn, and register the complete initial-child identity. From
...
- [x] Attribute SIGKILL only through authenticated complete-identity termination
      receipts, limited to 1,024 entries and 60 seconds. Restart, eviction, expiry,
      contention, or unavailable evidence leaves attribution unconfirmed.
...
- `krypton run` has deterministic argument, path, daemon-discovery, receipt,
  signal-forwarding, and exit-status tests. Child-tree containment/tests remain
  outstanding; initial-child supervision does not complete this phase.
